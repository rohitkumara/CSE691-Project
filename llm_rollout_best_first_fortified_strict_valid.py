...
The agent can perform only the following actions:
- Move to [object or location]
- Pick up [object]
- Drop [object]
- Toggle (open) [object]
...
Goal: {goal}.
Mission: {mission}.
Suggest next 3 valid subtasks.
"""
    else:
        prompt = f"""The agent has already completed: {prior_chain}.
Only suggest next valid subtasks using allowed actions (Move to, Pick up, Drop, Toggle).
Forbidden behaviors: push box, teleport, signal completion.
Mission: {mission}.
"""
...
def generate_plan_from_chain(subtask_chain, agent_pos, objects, goal, mission):
    prompt = f"""The agent must first perform: {subtask_chain}.
Then continue planning steps to complete the mission: {mission}.
Only allowed actions: Move to, Pick up, Drop, Toggle.
"""
