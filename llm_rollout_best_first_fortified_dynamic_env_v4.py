...
def best_first_fortified_rollout_planner():
    ...
    current_chain = []
    current_env = copy.deepcopy(env)  # Start a working copy
    best_plan = []
    best_score = float('inf')

    iteration = 0
    MAX_DEPTH = 20  # Prevent chains from growing too long
    done = False

    print("\n[Starting One-Step Rollout Search]\n")
    ...
    while not done and iteration < MAX_DEPTH:
        ...
        for subtask in subtasks:
            ...
            rollout_cost = execute_plan_with_gif(env_copy, plan, path, gif_name=f"rollouts/rollout_step_{iteration}_candidate.gif")
        ...
        # Apply the best subtask to real environment
        print(f"[Step {iteration}] Executing best subtask: {best_subtask}")
        current_chain.append(best_subtask)

        # Actually move the real environment forward
        plan = generate_plan_from_chain(current_chain, agent_pos, objects, goal, mission)
        path = PathPlanner(current_env, None)
        _ = execute_plan_with_gif(current_env, plan, path, gif_name=f"rollouts/rollout_step_{iteration}_best.gif")

        agent_pos = tuple(current_env.agent_pos)
        objects = extract_objects(current_env.grid)

        # Update best plan and best score if better
        if best_rollout_cost < best_score:
            best_score = best_rollout_cost
            best_plan = list(current_chain)
    ...
    print("\n[Final Best Plan Found]:", best_plan)
    ...
    final_steps = execute_plan_with_gif(final_env, plan, final_path, gif_name="rollouts/final_best_plan.gif")
