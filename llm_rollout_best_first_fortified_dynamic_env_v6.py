...
def best_first_fortified_rollout_planner():
    ...
    print("\n[Starting One-Step Rollout Search with True Expansion]\n")
    ...
    while not done and iteration < MAX_DEPTH:
        ...
        best_env_after_subtask = None  # 🌟 To store environment state after applying best subtask
        child_id = 0
        ...
        for subtask in subtasks:
            ...
            if rollout_cost < best_rollout_cost:
                ...
                best_env_after_subtask = copy.deepcopy(env_copy)  # 🌟 Save environment after best subtask

        if best_subtask is None:
            print("[No valid subtask led to improvement. Stopping.]")
            break

        # Apply the best subtask
        print(f"[Step {iteration}] Executing best subtask: {best_subtask}")
        current_chain.append(best_subtask)

        # 🌟 Update real environment to best_env_after_subtask
        current_env = copy.deepcopy(best_env_after_subtask)
    ...
    final_steps = execute_plan_with_gif(final_env, best_plan, final_path, gif_name="rollouts/final_best_plan.gif")
