...
def multi_branch_fortified_rollout_planner():
    env = SimpleEnv()
    obs, _ = env.reset()
    objects = extract_objects(env.grid)
    goal = obs["goal"]
    ...
    while queue:
        ...
        for chain in queue:
            # Expand current chain
            subtasks = get_subtasks(agent_pos, objects, goal, mission, prior_chain=chain if chain else None)
            if not subtasks:
                continue

            for subtask in subtasks:
                candidate_chain = chain + [subtask]
                candidate_id += 1
                ...
                # Always explore deeper if feasible
                new_queue.append(candidate_chain)

        if not new_queue:
            print("[No more expansions possible, terminating]")
            break
...
if __name__ == "__main__":
    multi_branch_fortified_rollout_planner()
